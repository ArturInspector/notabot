...
describe("GitcoinAdapter", function() {
  ...
  describe("verifyAndRegister", function() {
    it("should verify valid signature", async function() {
      ...
      ).to.emit(gitcoinAdapter, "GitcoinVerified");
    });
    ...
    it("should revert on invalid signature", async function() {
      ...
      await expect(
        gitcoinAdapter.verifyAndRegister(user.address, proof)
      ).to.be.revertedWithCustomError(gitcoinAdapter, "InvalidSignature");
    });
    ...
    it("should revert on low score", async function() {
      ...
      await expect(
        gitcoinAdapter.verifyAndRegister(user.address, proof)
      ).to.be.revertedWithCustomError(gitcoinAdapter, "ScoreTooLow");
    });
    ...
    it("should revert on expired proof", async function() {
      ...
      await expect(
        gitcoinAdapter.verifyAndRegister(user.address, proof)
      ).to.be.revertedWithCustomError(gitcoinAdapter, "ProofExpired");
    });
    ...
    it("should revert on duplicate userId", async function() {
      ...
      await expect(
        gitcoinAdapter.verifyAndRegister(user.address, proof)
      ).to.be.revertedWithCustomError(gitcoinAdapter, "ProofAlreadyUsed");
    });
  });
